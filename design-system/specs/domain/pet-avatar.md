...
- **Base:** Avatar with `ring=gender` (→ `color.semantic.domain.pet.male|female|unknown`), fallback glyph `dog`.
...
- **Name (optional, when labeled):** rendered in the gender color (`color.semantic.domain.pet.*`) — matches the app's name-tint rule.
- **Sizes:** inherit Avatar (`sm`–`xl`); `lg`/`xl` for profile, `sm`/`md` in stacks.
- **RN compatibility:** props `src`, `name`, `gender`, `isPuppy`, `size`, `showName?`.

## Acceptance Criteria
- [ ] Gender ring + name tint from `color.semantic.domain.pet.*` only.
