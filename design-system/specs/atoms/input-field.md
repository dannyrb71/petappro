...
- **States:**
  - default — fill `color.semantic.surface.bright`, `size.stroke.hairline` `color.semantic.border.default`, text `color.semantic.text.default`, placeholder `color.semantic.text.faint`.
  - focus — border `color.semantic.action.primary.default` + `size.stroke.focus` `color.semantic.focus-ring`.
...
- **Shape:** radius `radius.semantic.input`. Min height `size.control.md` (44); textarea min 92 (governed exception — multi-line default height; no control-height token applies). Padding from `spacing.primitive.3`/`4`.
...
- [ ] Label always associated (or documented aria-label); focus ring ≥3:1; ≥ `size.min-touch-target` (44) target.
...
- **Accessibility:** label association, error announced via `aria-describedby`, `size.min-touch-target` (44) target.
